...
public class MagnetismEffect extends MobEffect {
    public MagnetismEffect(MobEffectCategory category, int color) {
    ...
    public boolean applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        if (entity instanceof Player && !player.isCrouching()) {
            for (ItemEntity itemEntity : list) {
                ...
                if (player.getInventory().canPlaceItem(1, itemEntity.getItem()) && !itemEntity.hasPickUpDelay() && itemEntity.isAlive() && !level.isClientSide) {
